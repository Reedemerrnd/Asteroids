...
namespace Asteroids
{
    ...
    public class DictionaryDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (expanded)
            {
                ...
                for (int i = 0; i < cnt; i++)
                {
                    ...
                    var w0 = EditorGUIUtility.labelWidth; 
                    var w1 = r.width - w0;
        ...
        private static Color OffsetColor(Color value)
        {
            var colorOffset = 0.01f;
            if (value.r <= 0.99f)
            {
                value.r += colorOffset;
            }
            else if (value.g <= 0.99f)
            {
                value.g += colorOffset;
            }
            else if (value.b <= 0.99f)
            {
                value.b += colorOffset;
            }
            else
